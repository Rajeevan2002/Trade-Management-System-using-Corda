...
public class FlowTests {
    ...
    public void transactionConstructedByFlowUsesTheCorrectNotary() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,10,"xx");
    ...
    public void transactionConstructedByFlowHasOneTokenStateOutputWithTheCorrectAmountAndOwner() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,1,"LL");
        CordaFuture<SignedTransaction> future = nodeA.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
        ...
        assertEquals(nodeB.getInfo().getLegalIdentities().get(0), output.getConsumer());
    ...
    public void transactionConstructedByFlowHasOneOutputUsingTheCorrectContract() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,11,"lk");
        CordaFuture<SignedTransaction> future = nodeA.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
    ...
    public void transactionConstructedByFlowHasOneIssueCommand() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,14,"dk");
        CordaFuture<SignedTransaction> future = nodeA.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
    ...
    public void transactionConstructedByFlowHasOneCommandWithTheIssuerAndTheOwnerAsASigners() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,5,"oo");
        CordaFuture<SignedTransaction> future = nodeA.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
    ...
    public void transactionConstructedByFlowHasNoInputsAttachmentsOrTimeWindows() throws Exception {
        TokenIssueFlow.TokenIssueFlowInitiator flow = new TokenIssueFlow.TokenIssueFlowInitiator(nodeB.getInfo().getLegalIdentities().get(0), 99,5,"lk");
        CordaFuture<SignedTransaction> future = nodeA.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTransaction = future.get();
